@php($pageTitle = 'Tambah Peminjaman')
...
@section('content')
    <div class="content-panel mx-auto max-w-2xl rounded-2xl p-6">
        <div class="flex items-start justify-between gap-4 border-b border-slate-200 pb-5">
            <div>
                <h1 class="text-2xl font-semibold text-slate-900">Tambah Peminjaman</h1>
                <p class="mt-1 text-sm text-slate-500">Isi data peminjaman baru lalu simpan.</p>
            </div>
            <a href="{{ route('peminjaman.index') }}" class="rounded-md button-secondary px-4 py-2 text-sm font-medium">Kembali</a>
        </div>

        @if (session('success'))
            <div class="mt-5 rounded-md bg-emerald-100 p-3 text-sm text-emerald-800">
                {{ session('success') }}
            </div>
        @endif
        ...
        <form method="POST" action="{{ route('peminjaman.store') }}" class="mt-6 space-y-4">
            @csrf
            ...
            <div>
                ...
                <select id="buku_id" name="buku_id" class="w-full rounded-md border border-slate-300 px-3 py-2" required>
                    ...
                    @foreach ($buku as $item)
                        <option value="{{ $item->id }}" @selected(old('buku_id') == $item->id)>
            ...
            <div>
                ...
                <input id="tanggal_pinjam" type="date" name="tanggal_pinjam" value="{{ old('tanggal_pinjam') }}" class="w-full rounded-md border border-slate-300 px-3 py-2" required>
            </div>

            <div>
                <label for="tanggal_kembali" class="mb-1 block text-sm font-medium text-slate-700">Tanggal Kembali (opsional)</label>
                <input id="tanggal_kembali" type="date" name="tanggal_kembali" value="{{ old('tanggal_kembali') }}" class="w-full rounded-md border border-slate-300 px-3 py-2">
            </div>
            ...
            <div>
                ...
                <select id="status" name="status" class="w-full rounded-md border border-slate-300 px-3 py-2" required>
                    @foreach (['dipinjam', 'dikembalikan', 'terlambat'] as $status)
                        <option value="{{ $status }}" @selected(old('status', 'dipinjam') === $status)>
                            {{ ucfirst($status) }}
                        </option>
                    @endforeach
                </select>
            ...
            <button type="submit" class="w-full rounded-md button-primary px-4 py-2 font-medium">
                Simpan
            </button>
        </form>
    </div>
